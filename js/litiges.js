  ...
  document
    ...
    ?.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showMsg(
          'Erreur : ' +
            (err.message || 'Impossible d\'envoyer le signalement.'),
          'error'
        );
...
async function renderDetail(litigeId) {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="commandes-empty">
        <p>
          Litige introuvable, ou vous n'avez pas accès à celui-ci.
        </p>
      </div>
    `;
    return;
  ...
  content.innerHTML = `
      ...
      <div class="commande-item">
        ...
        <div class="commande-item-info">
          ...
          <p class="commande-item-detail">
            Ouvert le ${date} ·
            ${estAcheteur ? 'Vous êtes l\'acheteur' : 'Vous êtes le vendeur'}
          </p>
        </div>
    ...
    <section
      id="litigeAdminSection"
      class="litige-admin-section hidden"
    ></section>
...
async function renderAdminSection(litige) {
  ...
  const admin = await Litiges.isAdmin({
    supabaseClient,
    userId: currentUserId
  });
  ...
  section.innerHTML = `
    ...
    <form id="litigeAdminForm" class="litige-form">
      ...
      <select id="litigeAdminStatut" required>
        ...
        <option value="resolu_acheteur">
          En faveur de l'acheteur
        </option>
        <option value="resolu_vendeur">
      ...
      <label for="litigeAdminDecision">
        Motivation de la décision
      </label>

      <textarea
        id="litigeAdminDecision"
        rows="4"
        placeholder="Visible par l'acheteur et le vendeur."
        required
      ></textarea>
  ...
  document
    .getElementById('litigeAdminForm')
    ?.addEventListener('submit', async (e) => {
      e.preventDefault();

      const btn = document.getElementById(
        'btnLitigeAdminSubmit'
      );
      ...
      try {
        await Litiges.resolveLitige({
          ...
          decision: document.getElementById(
            'litigeAdminDecision'
          ).value.trim(),
          resolvedBy: currentUserId
        });

        await renderDetail(litige.id);
        ...
        msgEl.textContent =
          'Erreur : ' +
          (err.message || 'Impossible d\'enregistrer la décision.');
...
async function renderPreuves(preuves) {
  ...
  const container =
    document.getElementById('litigePreuvesList');
...
// ── INITIALISATION ──
async function init() {
  // supabaseClient doit être créé dans supabase.js,
  // chargé avant ce fichier.
  if (typeof supabaseClient === 'undefined') {
    console.error(
      'supabaseClient est introuvable. Vérifiez que supabase.js est chargé avant litige.js.'
    );
    return;
  }
  ...
  // Cart doit être chargé avant ce fichier.
